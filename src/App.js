...
function App() {
  ...
  return (
    <div className="App">
      ...
      />
      <button onClick={() => signInMethod(email, password)}>アカウント作成</button>
      ...
      />
      <button onClick={() => loginForm(loginEmail, loginPassword)}>ログイン</button>
      <h4>ログイン認証コード入力</h4>
      <div id="sign-in-button"></div>
      <input
        type="number"
      ...
      />
      <button onClick={() => loginVerifyAction(loginOpt)}>認証コード入力</button>
    </div>
  );
}
